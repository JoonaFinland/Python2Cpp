 ...
 * Copyright (c) 2019 Joona Jokivuori
 *
...
std::string KerasLayerActivation::Check(Tensor* in) {
...
bool KerasLayerActivation::Apply(Tensor* in, Tensor* out) {
    ...
    *out = *in;
	...
	case kSoftMax: {
		...
		//float max = *max_element(out->data_.begin(), out->data_.end());
		for (size_t i = 0; i < out->data_.size(); i++) {
...
std::string KerasLayerDense::Check(Tensor* in) {
	// input tensor needs to be same size as weights
	if (in->dims_[0] != weights_.dims_[0]) {
		std::string str = "Invalid input: expected ," + std::to_string(weights_.dims_[0]) + " but got ," + std::to_string(in->dims_[0]) + " instead.";
...
std::string KerasLayerConvolution2d::Check(Tensor* in) {
	// input tensor dimensions should match weights
	if (in->dims_[0] != weights_.dims_[0] || in->dims_[1] != weights_.dims_[1]) {
		std::string str = "Invalid input: expected ," + std::to_string(weights_.dims_[0]) + "," + std::to_string(weights_.dims_[1]) + " but got ," + std::to_string(in->dims_[0]) + "," + std::to_string(in->dims_[1]) + " instead.";
		return str;
	}
	return "true";
}
...
std::string KerasLayerFlatten::Check(Tensor* in) {
	// Always true, no dimension requirements
	return "true";
}
...
std::string KerasLayerElu::Check(Tensor* in) {
	// ELU in this case has no information about dimensionality requirements. Needs to be implemented differently
	return "true";
}
...
std::string KerasLayerMaxPooling2d::Check(Tensor* in) {
	// input tensor dimensions should be 3d
	if (in->dims_.size() != 3) {
		std::string str = "Invalid input: expected 3D, but got " + std::to_string(in->dims_.size()) + "D instead.";
		return str;
	}
	return "true";
}
...
std::string KerasLayerLSTM::Check(Tensor* in) {
	// LSTM input size must match weight size.
	if (in->dims_[0] != Wi_.dims_[0]) {
		std::string str = "Invalid input: expected ," + std::to_string(Wi_.dims_[0]) + " but got ," + std::to_string(in->dims_[0]) + " instead.";
		return str;
	}
	return "true";
}
...
std::string KerasLayerEmbedding::Check(Tensor* in) {
	// input tensor dimensions should match weights expected.
	if (in->dims_[0] != weights_.dims_[0]) {
		std::string str = "Invalid input: expected ," + std::to_string(weights_.dims_[0]) + " but got ," + std::to_string(in->dims_[0]) + " instead.";
		return str;
	}
